const kubioEnv = require( '../packages/env' );

const WORDPRESS_NAMESPACE = '@wordpress/';
const GUTENTAG_NAMESPACE = '@kubio/';
const KUBIO_CHILD_NAMESPACE = `${ kubioEnv.KUBIO_PACKAGE_PREFIX }/`;
...
function defaultRequestToExternal( request ) {
	...
	if ( request.startsWith( KUBIO_CHILD_NAMESPACE ) ) {
		return [
			kubioEnv.KUBIO_BLOCK_PREFIX,
			camelCaseDash(
				request
					.substring( KUBIO_CHILD_NAMESPACE.length )
					.split( '/' )[ 0 ]
			),
		];
	}
...
function defaultRequestToHandle( request ) {
	...
	if ( request.startsWith( KUBIO_CHILD_NAMESPACE ) ) {
		return (
			`${ kubioEnv.KUBIO_BLOCK_PREFIX }-` +
			request.substring( KUBIO_CHILD_NAMESPACE.length ).split( '/' )[ 0 ]
		);
	}
}
