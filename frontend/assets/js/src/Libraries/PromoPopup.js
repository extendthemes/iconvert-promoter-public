...
export const PromoPopup = (() => {
  ...
  const promoShow = (popupID) => {
    ...
    const animateContent = () => {
      ...
      const effect = popupContent.data("show-effect") ?? "animate__fadeIn";
  ...
  const promoClosePopup = (popupID) => {
    ...
    const showAnimate = popupContent.attr("data-show-effect");
    const exitAnimate = popupContent.attr("data-exit-effect");
    ...
    if (exitAnimate) {
      if (isAboveContent) {
        ...
        hideMainContainer(true, duration);
      } else {
        const isLightbox =
