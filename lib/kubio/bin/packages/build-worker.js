...
const packageImporter = (url) => {
	const matchPackageUrl = /^~@kubio/gim;
	if (!url.match(matchPackageUrl)) {
		return null;
	}
	const cleanedUrl = url.replace(matchPackageUrl, path.resolve(PACKAGES_DIR));
...
const BUILD_TASK_BY_EXTENSION = {
	async '.scss'(file) {
		...
		const builtSass = await renderSass({
			...
			data:
				'@use "sass:math";' +
				['wordpress-imports', 'variables', 'bem', 'mixins']
	...
	async '.js'(file) {
		for (const [environment, buildDir] of Object.entries(JS_ENVIRONMENTS)) {
			...
			let babelOptions = getBabelConfig(
				environment,
				file.replace(PACKAGES_DIR, '@kubio')
			);
