...
console.log(
	'Start syncing for',
	Object.keys( rootDependencies ).length,
	'packages'
);

Object.keys( rootDependencies ).forEach( async ( pack ) => {
	const folder = pack.replace( '@kubio/', '' );
			...
			const matches = content
				...
				.filter(
					( match ) =>
						match.indexOf( './' ) === -1 &&
						match.indexOf( '..' ) === -1 &&
						match.indexOf( 'element-ui/' ) === -1 &&
						match.indexOf( '.' ) === -1 &&
						match.indexOf( 'react-native' ) === -1 &&
						match.indexOf( 'build-types' ) === -1 
				);
	...
	imports.forEach( ( importedPackage ) => {
		if ( importedPackage.indexOf( '@kubio/' ) === 0 ) {
			...
			} else if ( rootDevDependencies[ importedPackage ] ) {
				packPackage.devDependencies[ importedPackage ] =
					rootDevDependencies[ importedPackage ];
			} else {
				try {
					...
					if ( ! rootDevDependencies[ importedPackage ] ) {
						rootDevDependencies[ importedPackage ] = response;
						console.log(
							'\t',
							importedPackage,
	...
	fs.writeFileSync(
		rootPackageJSONPath,
		JSON.stringify( rootPackageJSON, undefined, 4 )
	);
} );
