...
module.exports = {
	...
	entry,
	output: {
		devtoolNamespace: 'kubio',
		filename: './build/[name]/index.js',
		path: __dirname,
		library: [ 'kubio', '[name]' ],
		libraryTarget: 'this',
		devtoolModuleFilenameTemplate: ( info ) => {
			if ( info.resourcePath.includes( '/@kubio/' ) ) {
				const resourcePath = info.resourcePath.split( '/@kubio/' )[ 1 ];
