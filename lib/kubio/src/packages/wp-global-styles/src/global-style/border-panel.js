...
function useHasBorderColorControl(name) {
	...
	return (
		useSetting('border.color', name)[0] && supports.includes('borderColor')
	);
}
...
function useHasBorderStyleControl(name) {
	const supports = getSupportedGlobalStylesPanels(name);
	return (
		useSetting('border.style', name)[0] && supports.includes('borderStyle')
	);
}
...
function useHasBorderWidthControl(name) {
	const supports = getSupportedGlobalStylesPanels(name);
	return (
		useSetting('border.width', name)[0] && supports.includes('borderWidth')
	);
}
...
export default function BorderPanel({ name }) {
	...
	const [borderWidthValue, setBorderWidth] = useStyle('border.width', name);
	...
	return (
		<ToolsPanel label={__('Border', 'kubio')} resetAll={resetAll}>
			{showBorderWidth && (
				<ToolsPanelItem
