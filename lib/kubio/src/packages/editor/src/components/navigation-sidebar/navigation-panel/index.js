...
const NavigationPanel = ({ isOpen }) => {
	...
	const [templatesActiveMenu, setTemplateActiveMenu] = useState(MENU_ROOT);

	const { siteTitle, openedMenu } = useSelect((select) => {
		...
		return {
			...
			openedMenu: select(STORE_NAME).getNavigationMenu(),
		};
	}, []);
	...
	useEffect(() => {
		if (openedMenu && isOpen && !prevIsOpen) {
			setContentActiveMenu(openedMenu);
		}
	}, [contentActiveMenu, isOpen, openedMenu]);
			...
			<div className="edit-site-navigation-panel__inner">
				<div className="edit-site-navigation-panel__site-title-container">
					<Flex>
						...
						<FlexItem>
							<Tooltip text={__('Close panel', 'kubio')}>
								<Button
									className={
				...
				<div className="edit-site-navigation-panel__scroll-container">
					{templatesActiveMenu === MENU_ROOT && (
						<ContentNavigation
							...
							activeMenu={contentActiveMenu}
						/>
					)}
					{contentActiveMenu === MENU_ROOT && (
						<TemplatesNavigation
							activeMenu={templatesActiveMenu}
							onActivateMenu={setTemplateActiveMenu}
						/>
					)}
				</div>
			</div>
