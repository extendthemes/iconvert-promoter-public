	...
	if (_creator) {
		_caption = title
			? sprintf(
					...
					_x('"%1$s" by %2$s/ %3$s', 'caption', 'kubio'),
			...
			: sprintf(
					...
					_x('<a %1$s>Work</a> by %2$s/ %3$s', 'caption', 'kubio'),
		...
		_caption = title
			...
			: sprintf(
					...
					_x('<a %1$s>Work</a>/ %3$s', 'caption', 'kubio'),
					getExternalLinkAttributes(foreignLandingUrl),
					licenseUrl
						? getExternalLink(
								`${licenseUrl}?ref=openverse`,
...
const inserterMediaCategories = [
	{
		name: 'images',
		labels: {
			name: __('Images', 'kubio'),
			search_items: __('Search images', 'kubio'),
		},
		mediaType: 'image',
		async fetch(query = {}) {
	...
	},
	{
		name: 'videos',
		labels: {
			name: __('Videos', 'kubio'),
			search_items: __('Search videos', 'kubio'),
		},
		mediaType: 'video',
		async fetch(query = {}) {
	...
	},
	{
		...
		labels: {
			name: __('Audio', 'kubio'),
			search_items: __('Search audio', 'kubio'),
		},
		mediaType: 'audio',
		async fetch(query = {}) {
	...
	},
	{
		name: 'openverse',
		labels: {
			name: __('Openverse', 'kubio'),
			search_items: __('Search Openverse', 'kubio'),
		},
		mediaType: 'image',
		async fetch(query = {}) {
			const defaultArgs = {
