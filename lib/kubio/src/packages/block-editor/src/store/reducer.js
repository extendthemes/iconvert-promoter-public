...
export function isUpdatingSameBlockAttribute( action, lastAction ) {
	return false; // modified in kubio to ensure constant save triggering
}
...
// added in kubio
export function lastBlockRemoved( state = {}, action ) {
	switch ( action.type ) {
		case 'RESET_BLOCKS':
			return {
				clientIds: [],
		...
		// maybe we should reset that state to {} here 🤷
		case 'INSERT_BLOCKS':
		case 'RECEIVE_BLOCKS':
			if ( ! action.blocks.length ) {
				return state;
			...
			const foundDeleted = action.blocks.filter(
				( block ) =>
					( state.clientIds || [] ).indexOf( block.clientId ) !== -1
			);
			return {
				clientIds: difference( state.clientIds || [], foundDeleted ),
			};
		case 'REMOVE_BLOCKS':
