 ...
 */
import { castArray, isEmpty, isObject, isString } from 'lodash';
 ...
 */
import { isGutentagPrefixed, wpVersionCompare } from '@kubio/utils';
...
const KUBIO_DEFAULT_LOCKED_PROPS = {
	move: true,
	remove: true,
};

const KUBIO_DEFAULT_LOCKED_BLOCKS = [
	'core/post-content',
	'kubio/header',
	'kubio/footer',
	'kubio/query-layout',
...
export const getBlockAttributes = createRegistrySelector(
	( select ) => ( state, clientId ) => {
		...
		const attrs = state.blocks.attributes.get( clientId );

		const blockName = getBlockName( state, clientId );

		const defaultLocked = applyFilters(
			'kubio.default_locked_blocks',
			KUBIO_DEFAULT_LOCKED_BLOCKS
		);

		const isLockedByFilter = applyFilters(
			'kubio.is_block_locked',
			false,
			block,
			select
		);
		...
		if ( !! isLockedByFilter ) {
			if ( isObject( isLockedByFilter ) ) {
				attrs.lock = isLockedByFilter;
			} else {
				attrs.lock = KUBIO_DEFAULT_LOCKED_PROPS;
			}
		} else if ( defaultLocked.includes( blockName ) ) {
			attrs.lock = attrs.lock || KUBIO_DEFAULT_LOCKED_PROPS;
		}
...
export function getBlocks( state, rootClientId ) {
	if (
		! isEmpty( rootClientId ) &&
		isEmpty( state.blocks.order.get( rootClientId ) )
	) {
		return EMPTY_ARRAY;
	}
...
export function getBlockSelectionStart( state ) {
	const clientId = state.selection.selectionStart.clientId;

	if ( ! state.blocks.byClientId.get( clientId ) ) {
		return null;
	}
...
export function getBlockSelectionEnd( state ) {
	const clientId = state.selection.selectionEnd.clientId;
...
export const getBlockParents = createSelector(
	( state, clientId, ascending = false ) => {
		const parents = [];
		//modified in kubio
		let current = castArray( clientId )[ 0 ];
...
export const getInserterItems = createSelector(
	( state, rootClientId = null ) => {
		...
		const items = blockTypeInserterItems.reduce( ( accumulator, item ) => {
			...
			if ( variations.length ) {
				...
				variations.forEach( ( variation ) => {
					if (
						! variation.isDefault &&
						isGutentagPrefixed( item.name )
					) {
						return;
					}

					accumulator.push( variationMapper( variation ) );
				} );
			}
			return accumulator;
		}, [] );
		...
		let resultedItems = applyFilters(
			'kubio.editor.inserterItems',
			[ ...sortedBlockTypes, ...syncedPatternInserterItems ],
			state,
			rootClientId
		);
	...
	( select ) =>
		createSelector(
			( state, patternName ) => {
				...
				// @ATTENTION modified in Kubio - the Kubio patterns are already parsed
				let blocks = pattern.content;
				if ( isString( blocks ) ) {
...
//modified in kubio for 6.4 compatibility
...
export const __experimentalGetAllowedPatterns65 = createSelector(
	( state, rootClientId = null ) => {
...
const getPatternsByBlockTypes64 = createSelector(
	( state, blockNames, rootClientId = null ) => {
		if ( ! blockNames ) return EMPTY_ARRAY;
		const patterns = __experimentalGetAllowedPatterns65(
			state,
			rootClientId
		);
	...
	( state, blockNames, rootClientId ) => [
		...__experimentalGetAllowedPatterns65.getDependants(
			state,
			rootClientId
		),
	]
...
const getPatternsByBlockTypes65 = createRegistrySelector( ( select ) =>
	createSelector(
		( state, blockNames, rootClientId = null ) => {
			if ( ! blockNames ) {
				return EMPTY_ARRAY;
...
export const getPatternsByBlockTypes = isWp64
	? getPatternsByBlockTypes64
	: getPatternsByBlockTypes65;
// end modified in kubio for 6.4 compatibility
...
// added in kubio
export function wasBlockJustRemoved( state, clientId ) {
	const { lastBlockRemoved } = state;
	return lastBlockRemoved.clientIds?.includes( clientId );
}

export {getSectionRootClientId};
