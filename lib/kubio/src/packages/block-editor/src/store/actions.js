...
export const selectBlock =
	( clientId, initialPosition = 0 ) =>
	( { select, dispatch } ) => {
		const block = select.getBlock( clientId );

		if ( applyFilters( 'kubio.can_select_block', true, block, select ) ) {
			/* eslint-enable jsdoc/valid-types */
			return dispatch( {
				type: 'SELECT_BLOCK',
				initialPosition,
...
export function showInsertionPoint(
	...
	const { __unstableWithInserter, operation, kubioInsertionEdge } =
		__unstableOptions;
	return {
...
export const __unstableSetEditorMode =
	...
	( { dispatch, select } ) => {
		...
		if ( mode === 'navigation' ) {
			...
			speak( __( 'You are currently in zoom-out mode.', 'kubio' ) );
		}
	};
