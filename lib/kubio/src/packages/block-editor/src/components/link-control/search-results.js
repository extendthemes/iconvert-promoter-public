...
const preventScrollPropagationRef = (node)=>{
	if (node) {
		node.addEventListener('wheel', preventScrollPropagation, {
			passive: false,
		});
	}
	return () => {
...
export default function LinkControlSearchResults( {
	...
	const suggestionsListPropsRef = useMergeRefs([
		suggestionsListProps.ref,
		preventScrollPropagationRef,
	])
