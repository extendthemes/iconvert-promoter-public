...
function BlockPopover(
	...
	let selectedElement = useBlockElement( clientId );

	//for kubio loop block as it does not work here. We try to find the node using a basic querySelector
	if ( ! selectedElement ) {
		const iframe = document.querySelector( '#kubio-iframe-holder iframe' );
		const iframeDocument = iframe?.contentDocument;
		if ( iframeDocument ) {
			selectedElement = iframeDocument.querySelector(
				`[data-block="${ clientId }"]`
			);
		}
	}
