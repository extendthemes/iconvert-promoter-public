...
import { cleanFontFamilyFromBlocks } from '@kubio/utils';
...
export function onBlockDrop(
	...
	return ( event ) => {
		...
		if ( dropType === 'inserter' ) {
			...
			insertOrReplaceBlocks( blocksToInsert, selectBlock, null );
		}
...
export default function useOnBlockDrop(
	...
	const insertOrReplaceBlocks = useCallback(
		(
			...
			blocks = cleanFontFamilyFromBlocks( blocks );
			...
			if ( operation === 'replace' ) {
				...
				insertBlocks(
					...
					targetBlockIndex,
					targetRootClientId,
					uiVersion === 1 ? updateSelection : false,
					initialPosition
				);
			}
		},
