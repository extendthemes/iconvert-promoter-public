	...
	const onClick = useCallback(() => {
		...
		createSuccessNotice(
			sprintf(
				...
				__('Template Part "%s" inserted.', 'kubio'),
				title || slug
			),
			{
				type: 'snackbar',
...
function TemplatePartSearchResults({
	...
	const filteredTPs = useMemo(() => {
		...
		searchResults.sort((a, b) => {
			...
			// Since diacritics can be used in theme names, remove them for the comparison.
			return (
				deburr(a.theme).toLowerCase().indexOf(normalizedFilterValue) -
	...
	return filteredTPs.map((templatePart) => (
		<PanelGroup
			key={templatePart.id}
			title={templatePart.theme || __('Custom', 'kubio')}
		>
...
export default function TemplatePartPreviews({
	...
	let templateParts = useSelect((select) => {
		return (
	...
	if (blockArea) {
		templateParts = templateParts.filter((element) => {
			if (element.area === blockArea) return true;
			return false;
		});
	}
	...
	if (filterValue) {
		return (
			<Composite
				{...composite}
				role="listbox"
				aria-label={__('List of template parts', 'kubio')}
			>
