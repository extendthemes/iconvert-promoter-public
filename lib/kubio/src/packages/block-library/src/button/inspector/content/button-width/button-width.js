...
const ButtonWidth_ = ({ computed }) => {
	...
	return (
		<>
			...
			{buttonWidthIs.fit && (
				<RangeWithUnitControl
					label={__('Horizontal padding', 'kubio')}
			...
			<RangeWithUnitControl
				label={__('Vertical padding', 'kubio')}
...
const useComputed = (dataHelper,{computedButtonWidth = null}) => {

	if(computedButtonWidth){
		return computedButtonWidth;
	}
	...
	const textAlign = dataHelper.useStylePath(
		'justifyContent',
		{
...
const ButtonWidth = compose(withColibriDataAutoSave(useComputed))(ButtonWidth_);
export { ButtonWidth };
