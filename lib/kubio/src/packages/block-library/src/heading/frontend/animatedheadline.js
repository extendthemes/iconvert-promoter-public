(function ($) {
	const animatedHeadline = function (options) {
		...
		let duration = settings.animationDelay;
		...
		function hideWord(word) {
			...
			if (word.parents('.ah-headline').hasClass('type')) {
				const parentSpan = word.parent('.ah-words-wrapper');
	...
	// if in preview, add a small delay ( let the styling complete )
	if (window.wp && window.wp.customize) {
		$.fn.animatedHeadline = function () {
			const self = this,
				args = arguments;
			setTimeout(() => {
				animatedHeadline.apply(self, args);
			}, 100);
		};
	} else {
		$.fn.animatedHeadline = animatedHeadline;
	}
})(jQuery);
