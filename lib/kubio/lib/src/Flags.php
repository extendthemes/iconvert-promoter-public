...
class Flags {
	...
	private function __construct() {
		...
		$this->flags = get_option( Utils::getStringWithNamespacePrefix( '__kubio_instance_flags' ), array() );
	...
	private function withFlags( $action, $flag = null, $data = null ) {
		...
		if ( $action === 'get' ) {
			...
			$kubio_flags_defaults = apply_filters( Utils::getStringWithNamespacePrefix( 'kubio/instance-flags-default' ), array() );
	...
	public function save() {
		if ( $this->is_dirty_value ) {
			update_option( Utils::getStringWithNamespacePrefix( '__kubio_instance_flags' ), $this->flags, false );
		}
	}
