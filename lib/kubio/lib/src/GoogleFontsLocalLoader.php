...
class GoogleFontsLocalLoader {
	...
	private $queries_transient = null;
	...
	public function __construct() {
		...
		$this->queries_transient = Utils::getStringWithNamespacePrefix( 'kubio_local_google_queries_transient' );
		$this->uploads_dir       = Utils::getStringWithNamespacePrefix( 'kubio-google-fonts-cache' );
	...
	public function enqueueFonts( $query ) {
		...
		wp_enqueue_style(
			Utils::getPrefixedScriptName( 'local-google-fonts' ),
			add_query_arg(
				array(
	...
	public function getFontsQuery() {
		...
		$fonts = apply_filters( Utils::getStringWithNamespacePrefix( 'kubio/google_fonts' ), $fonts );
	...
	public function resolveFont() {
		...
		if ( ! $valid_nonce ) {
			wp_die( esc_html( 'Forbidden', 'iconvert-promoter' ), 403 );
		}
