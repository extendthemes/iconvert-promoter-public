...
class Importer {
	...
	public static function entityExists( $slug, $type = 'wp_template' ) {
	...
	public static function createTemplatePart( $slug, $content, $override = false, $source = 'theme' ) {
		...
		\KPromo\kubio_register_wp_template_part_area_taxonomy();
	...
	public static function getTemplateContent( $type, $slug ) {
		...
		switch ( $type ) {
			case 'page':
				...
				$path = apply_filters( Utils::getStringWithNamespacePrefix( 'kubio/importer/page_path' ), $path, $slug );
				break;
			case 'wp_template':
				$templates = Importer::getAvailableTemplates();
				$path      = Arr::get( $templates, $slug, null );
				$path      = apply_filters( Utils::getStringWithNamespacePrefix( 'kubio/importer/wp_template_path' ), $path, $slug );
				break;

			case 'wp_template_part':
				$templates = Importer::getAvailableTemplateParts();
				$path      = Arr::get( $templates, $slug, null );
				$path      = apply_filters( Utils::getStringWithNamespacePrefix( 'kubio/importer/wp_template_part_path' ), $path, $slug );
				break;
		}
		...
		return apply_filters( Utils::getStringWithNamespacePrefix( 'kubio/importer/content' ), $content, $type, $slug );
	}
	...
	public static function getAvailableTemplates( $extra_paths = array() ) {
		...
		return apply_filters( Utils::getStringWithNamespacePrefix( 'kubio/importer/available_templates' ), $result );
	}
	...
	public static function getAvailableTemplateParts( $extra_paths = array() ) {
		...
		return apply_filters( Utils::getStringWithNamespacePrefix( 'kubio/importer/available_template_parts' ), $result );
	}
	...
	public static function importRemoteFile( $source_url ) {
		...
		$replacement = untrailingslashit( apply_filters( Utils::getStringWithNamespacePrefix( 'kubio/importer/kubio-url-placeholder-replacement' ), '' ) );
		...
		$source_url = apply_filters( Utils::getStringWithNamespacePrefix( 'kubio/importer/kubio-source-url' ), $source_url );

		if ( apply_filters( Utils::getStringWithNamespacePrefix( 'kubio/importer/skip-remote-file-import' ), false ) ) {
			return array(
				'url' => $source_url,
				'id'  => 0,
			);
	...
	public static function maybeImportBlockAssets( $blocks, $on_time_expired = null ) {
		...
		require_once \KPromo\KUBIO_ROOT_DIR . '/lib/importer/assets-importer-filters.php';
		...
		foreach ( $blocks as $index => $block ) {
			...
			$block = apply_filters( Utils::getStringWithNamespacePrefix( 'kubio/importer/maybe_import_block_assets' ), $block, $block->blockName );
