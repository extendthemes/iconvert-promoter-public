...
class Separator extends Element {
	...
	public function __construct( $tag_name, $props, $children, $block ) {
		...
		$top               = $position === 'top';
		$shouldUseNegative = $negative && file_exists( \KPromo\KUBIO_ROOT_DIR . "lib/shapes/separators/{$type}-negative.svg" );

		$supportsNegative = file_exists( \KPromo\KUBIO_ROOT_DIR . "lib/shapes/separators/{$type}-negative.svg" );
		...
		$html = file_get_contents( \KPromo\KUBIO_ROOT_DIR . 'lib/shapes/separators/' . $type . '.svg' );
		$this->extendProps(
			array(
