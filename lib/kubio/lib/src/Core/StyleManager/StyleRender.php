...
class ElementConfig extends BasicPropsConfig {
	...
	public function getStateConfig( $state ) {
		$props = LodashBasic::merge(
			LodashBasic::get( $this->statesById, $state, array( 'selector' => ":{$state}" ) ),
...
class StyleRender {
	...
	public $prefixSelectorsByType = array(
		'shared'  => '#kubio',
		'local'   => '#kubio',
		'dynamic' => 'html',
		'global'  => '[data-kubio="cspromo/promopopup"]',
	);
	...
	public function __construct( $options ) {
		...
		$prefixSelectorsByType       = LodashBasic::get( $options, 'prefixSelectorsByType', array() );
		$prefixSelectorsByType       = apply_filters( CoreUtils::getStringWithNamespacePrefix( 'kubio/StyleRender/prefixSelectorsByType' ), $prefixSelectorsByType );
		$this->prefixSelectorsByType = \array_merge( $this->prefixSelectorsByType, $prefixSelectorsByType );
	...
	public function convertStyleToCss( $style, $settings ) {
		...
		$rootPrefix = LodashBasic::get(
			$settings,
			'prefix',
			$this->prefixSelectorsByType[ $styleType ]
		);
	...
	public function componentInstanceClass(
		...
		$style_prefix = apply_filters(
			CoreUtils::getStringWithNamespacePrefix( 'kubio/element-style-class-prefix' ),
			'style-'
		);
