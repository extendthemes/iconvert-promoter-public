...
class NotificationsManager {
	...
	private static $remote_data_url_base = 'https://kubiobuilder.com/wp-json/wp/v2/notificfation';

	public static function load() {
		static::$remote_data_url_base = apply_filters(
			Utils::getStringWithNamespacePrefix( 'kubio/NotificationsManager/remote_data_url_base' ),
			static::$remote_data_url_base
		);
	...
	public static function init() {
		...
		add_action( Utils::getKubioAjaxWithPrefix( 'wp_ajax_kubio-remote-notifications-retrieve' ), array( NotificationsManager::class, 'updateNotificationsData' ) );
	}
		...
		add_action(
			...
			function () {
				$fetch_url = add_query_arg(
					array( 'action' => Utils::getKubioUrlWithRestPrefix( 'kubio-remote-notifications-retrieve' ) ),
	...
	private static function getTransientKey() {
		$transient = Utils::getStringWithNamespacePrefix( 'kubio_remote_notifications' );
		if ( kubio_is_pro() ) {
			$transient = Utils::getStringWithNamespacePrefix( 'kubio_pro_remote_notifications' );
		}
