...
add_action(
	Utils::getStringWithNamespacePrefix( 'kubio/preview/handle_custom_entities' ),
	function ( $data ) {
		...
		if ( $kind === 'kubio' && $name === 'menu' ) {
			...
			add_filter(
				...
				function ( $menu_items, $current_menu, $args ) use ( $items, $menu ) {
					...
					if ( $cache = wp_cache_get( intval( $current_menu->term_id ), Utils::getStringWithNamespacePrefix( 'kubio/preview-menus' ) ) ) {
						return $cache;
					}
...
add_filter(
	...
	__NAMESPACE__ . '\\kubio_nav_menu_locations_preview'
);
