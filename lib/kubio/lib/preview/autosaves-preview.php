...
function kubio_handle_autosaved_posts_and_templates() {
	...
	$template_part_blocks = apply_filters( Utils::getStringWithNamespacePrefix( 'kubio/preview/template_part_blocks' ), array() );
	$context_based_blocks = apply_filters(
		Utils::getStringWithNamespacePrefix( 'kubio/preview/context_part_blocks' ),
		array_merge(
			array(
				'core/post-content',
			),
	...
	add_filter(
		Utils::getStringWithNamespacePrefix( 'kubio/template/template-loader-callback' ),
		function ( $callback ) use ( $autosaved_posts, $page_templates_remap ) {
			return function ( $template, $type, $templates ) use ( $autosaved_posts, $callback, $page_templates_remap ) {
				...
				if ( Str::endsWith( $template_file, 'template-canvas.php' ) ) {
					...
					if ( ( $kubio_template_source === 'kubio' || $kubio_template_source === 'kubio-custom' ) && function_exists( __NAMESPACE__ . '\\kubio_dequeue_theme_styles' ) ) {
						add_action( 'wp_print_styles', __NAMESPACE__ . '\\kubio_dequeue_theme_styles', PHP_INT_MAX );
					}
