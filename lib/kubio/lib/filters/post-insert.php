...
function kubio_import_assets_filter() {
	$post_types = array( 'cs-promo-popups' );

	foreach ( $post_types as $post_type ) {
		add_filter( "rest_pre_insert_{$post_type}", __NAMESPACE__ . '\\kubio_rest_pre_insert_import_assets' );
	}
}

add_action( 'init', __NAMESPACE__ . '\\kubio_import_assets_filter' );
