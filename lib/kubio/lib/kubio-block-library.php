...
function kubio_prefix_block_category_title( $category_title ) {
	...
	$prefix = __( 'Iconvert Promoter - %s', 'iconvert-promoter' );
...
function kubio_block_categories( $categories ) {
	...
	$kubio_categories = array(
		array(
			'slug'  => 'kubio-basic',
			'title' => __( 'Basic Blocks', 'iconvert-promoter' ),
		),
		array(
			'slug'  => 'kubio-components',
			'title' => __( 'Advanced Blocks', 'iconvert-promoter' ),
		),
		array(
			'slug'  => 'kubio-site-data',
			'title' => __( 'Site Data Blocks', 'iconvert-promoter' ),
		),
		array(
			'slug'  => 'kubio-blog-components',
			'title' => __( 'Blog Blocks', 'iconvert-promoter' ),
		),

		array(
			'slug'  => 'kubio-layout',
			'title' => __( 'Layout', 'iconvert-promoter' ),
		),

		array(
			'slug'  => 'kubio-template-parts',
			'title' => __( 'Template Parts', 'iconvert-promoter' ),
		),

	);

	$add_kubio_prefix = apply_filters( Utils::getStringWithNamespacePrefix( 'kubio/kubio_block_categories/add_kubio_prefix' ), true );
	...
	if ( $add_kubio_prefix ) {
		...
		$prefixed_categories = $kubio_categories;
	}
...
if ( kubio_is_kubio_editor_page() ) {
	Utils::onlyRunOnceForAllKubio( 'block_categories_all', 'kubio_block_categories' );
	add_filter( 'block_categories_all', __NAMESPACE__ . '\\kubio_block_categories', 10, 1 );
}
...
function kubio_get_block_metadata_mixin( $mixin ) {
	...
	if ( file_exists( $mixin ) ) {
		...
		$metadata = apply_filters( Utils::getStringWithNamespacePrefix( 'kubio/kubio_get_block_metadata_mixin/metadata' ), $metadata );
		return $metadata;
	}
...
function kubio_can_register_block( $block_name ) {
	...
	if ( in_array( $block_name, $kubio_editor_only_blocks, true ) ) {
		return false;
	}
...
function kubio_no_block_manifest_notice() {
	...
	<div class="notice notice-error">
		<p><?php esc_html_e( 'KPromo Error: Blocks manifest file (blocks-manifest.php) does not exists. Please recompile the plugin', 'iconvert-promoter' ); ?> </p>
	</div>
	<?php
}

function kubio_register_block_types_with_root_dir() {
	kubio_register_block_types( KUBIO_ROOT_DIR );
}

function kubio_register_block_types( $root_dir ) {
	$library_dir   = $root_dir . '/build/block-library/';
	$manifest_file = $library_dir . '/blocks-manifest.php';

	if ( ! file_exists( $manifest_file ) ) {
		if ( is_admin() ) {
			add_action( 'admin_notices', __NAMESPACE__ . '\\kubio_no_block_manifest_notice' );
		}

		return;
	}

	$kubio_autoloader = Utils::$kubio_autoloader;

	$blocks_dir      = $library_dir . '/blocks';
	$blocks_manifest = require_once $manifest_file;

	$blocks_namespace = 'KPromo\\Blocks';
...
add_action( 'init', __NAMESPACE__ . '\\kubio_register_block_types_with_root_dir', 9 );
...
if ( kubio_is_kubio_editor_page() ) {
	Utils::onlyRunOnceForAllKubio( 'enqueue_block_editor_assets', 'kubio_enqueue_editor_assets' );
}
...
function kubio_serialize_blocks( $blocks ) {
	...
	return implode( '', array_map( __NAMESPACE__ . '\\kubio_serialize_block', $blocks ) );
}
...
function kubio_serialize_block( $block ) {
	...
	if ( Utils::isKubioBlock( $block ) && $kubio_attr ) {
